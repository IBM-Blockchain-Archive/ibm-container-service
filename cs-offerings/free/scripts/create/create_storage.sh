...
if [ "${1}" == "--paid" ]; then
	if [ "$(kubectl get pvc | grep shared-pvc-paid | wc -l | awk '{ print $1 }')" == "0" ]; then
		echo "The paid PVC does not seem to exist"
		echo "Creating PVC named shared-pvc-paid"
		...
		while [ "$(kubectl get pvc | grep shared-pvc-paid | wc -l | awk '{ print $1 }')" == "0" ];
		do
			echo "Waiting for storage to be created"
			sleep 5
		done
	else
		echo "The paid PVC exists, not creating again"
	fi
else
